...
class CustomThemeFileReplacerTest < ActiveSupport::TestCase
  def setup
    @file_replacer = BulletTrain::Themes::Densha::CustomThemeFileReplacer.new("foo")
  end

  test "default_files_to_replace returns proper files" do
    default_files = [
      "./local/bullet_train-themes-foo/bullet_train-themes-densha.gemspec",
      "./local/bullet_train-themes-foo/app/assets/config/bullet_train_themes_densha_manifest.js",
      "./local/bullet_train-themes-foo/lib/tasks/bullet_train/themes/densha_tasks.rake",
      "./local/bullet_train-themes-foo/test/bullet_train/themes/densha_test.rb"
    ]
    assert_equal @file_replacer.default_files_to_replace("densha"), default_files
  end
  ...
  test "build_main_app_file_name returns file name that exists in the main app" do
    ...
    custom_gem_file = "./app/javascript/application.densha.js"

    # The file in the main app HAS been renamed, so we build it here.
    main_app_file = @file_replacer.build_main_app_file_name("densha", "foo", custom_gem_file)
