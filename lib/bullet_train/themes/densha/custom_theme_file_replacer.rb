  ...
  module Themes
    module Densha
      class CustomThemeFileReplacer
        ...
        include BulletTrain::Themes::Densha::FileReplacer
        ...
        def replace_theme(original_theme, custom_theme)
          ...
          files_to_replace.each do |custom_gem_file|
            ...
            begin
              if custom_gem_file[:must_compare] && !BulletTrain::Themes::Densha::FileReplacer.files_have_same_content?(custom_gem_file[:file_name], main_app_file)
                BulletTrain::Themes::Densha::FileReplacer.replace_content(old: custom_gem_file[:file_name], new: main_app_file)
              end
            rescue Errno::ENOENT => _
              puts "Skipping `#{main_app_file}` because it isn't present."
            end

            # Only rename file names that still have the original theme in them, i.e. - ./tailwind.config.densha.js
          ...
          # i.e. - `module Densha` and `tailwind.densha.config`.
            ...
            File.open(file, "r") do |f|
              ...
              new_lines = new_lines.map do |line|
                ...
                if f.path == "#{@repo_path}/lib/bullet_train/themes/#{custom_theme}.rb" && line.match?("class Theme < BulletTrain::Themes::")
                  ...
                  # `_account.html.erb` and `_devise.html.erb` have tailwind classes that contain `densha`.
                  ...
                  line.gsub!(original_theme, custom_theme) unless line.match?("bg-densha-gradient")
        ...
        # These files represent ones such as "./lib/bullet_train/themes/densha.rb" which
        ...
        # theme stay the same, i.e. - tailwind.densha.config.js. However, the names have
