  ...
  module Themes
    module Application
      def self.eject_theme(theme_name, ejected_theme_name)
        ...
        %x(sed -i #{'""' if `echo $OSTYPE`.include?("darwin")} "s/densha/#{ejected_theme_name}/g" #{Rails.root}/app/assets/stylesheets/#{ejected_theme_name}.tailwind.css)
...

        {
          ...
          "bullet_train-themes-densha" => "densha"
        ...
        %x(sed -i #{'""' if `echo $OSTYPE`.include?("darwin")} "s/densha/#{ejected_theme_name}/g" #{Rails.root}/test/system/resolver_system_test.rb)
      ...
      def self.release_theme(original_theme_name, args)
        if original_theme_name != "densha"
          puts "You can only release new themes based off of Bullet Train's Densha theme. Please eject a new theme from there, and publish your gem once you've finished making changes.".red
          exit 1
        end
        ...
        `git clone git@github.com:bullet-train-co/bullet_train-themes-densha.git ./local/bullet_train-themes-#{args[:theme_name]}`

        custom_file_replacer = BulletTrain::Themes::Densha::CustomThemeFileReplacer.new(args[:theme_name])
        custom_file_replacer.replace_theme("densha", args[:theme_name])
