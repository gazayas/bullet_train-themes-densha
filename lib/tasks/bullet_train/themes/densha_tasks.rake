...
namespace :bullet_train do
  namespace :themes do
    namespace :densha do
      desc "Fork the \"Densha\" theme into your local repository."
      ...
      desc "List view partials in theme that haven't changed since ejection from \"Densha\"."
      task :clean, [:theme] => :environment do |task, args|
        BulletTrain::Themes::Application.clean_theme(get_theme_name_from_task(task), args)
      end

      # Grabs the theme name from task, i.e. - bullet_train:theme:densha:eject.
